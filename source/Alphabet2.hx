package; //I... Could've just done this the whole time huh?
 ...
 */
class Alphabet2 extends FlxSpriteGroup
{
	...
	var listOAlphabets:List<AlphaCharacter2> = new List<AlphaCharacter2>();
	...
	public function quote(quote, xScale:Float = 1, yScale:Float = 1)
	{
		for (i in listOAlphabets)
			remove(i);
		_finalText = text;
	...
	public function addText()
	{
		...
		for (character in splitWords)
		{
			...
			if (AlphaCharacter2.alphabet.indexOf(character.toLowerCase()) != -1)
				// if (AlphaCharacter2.alphabet.contains(character.toLowerCase()))
			{
				if (lastSprite != null)
				{
				...
				// var letter:AlphaCharacter2 = new AlphaCharacter2(30 * loopNum, 0);
				var letter:AlphaCharacter2 = new AlphaCharacter2(xPos, 0);
	...
	public function startTypedText():Void
	{
		...
		new FlxTimer().start(0.05, function(tmr:FlxTimer)
		{
			...
			#if (haxe >= "4.0.0")
			var isNumber:Bool = AlphaCharacter2.numbers.contains(splitWords[loopNum]);
			var isSymbol:Bool = AlphaCharacter2.symbols.contains(splitWords[loopNum]);
			#else
			var isNumber:Bool = AlphaCharacter2.numbers.indexOf(splitWords[loopNum]) != -1;
			var isSymbol:Bool = AlphaCharacter2.symbols.indexOf(splitWords[loopNum]) != -1;
			#end

			if (AlphaCharacter2.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1 || isNumber || isSymbol)
				// if (AlphaCharacter2.alphabet.contains(splitWords[loopNum].toLowerCase()) || isNumber || isSymbol)

			{
				...
				// var letter:AlphaCharacter2 = new AlphaCharacter2(30 * loopNum, 0);
				var letter:AlphaCharacter2 = new AlphaCharacter2(xPos, 55 * yMulti);
				listOAlphabets.add(letter);
				letter.row = curRow;
				if (isBold)
				{
...
class AlphaCharacter2 extends FlxSprite
{
	...
	public function createBold(letter:String)
	{
		...
		animation.curAnim.frameRate = 24 * (60 / 120);
		updateHitbox();
	}
