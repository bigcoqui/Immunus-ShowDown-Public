...
class PlayState extends MusicBeatState
{
	...
	var shakingMode:Bool = false;
	var shakingIntensity:Float = 0;
	...
	public static var fixicon:Bool = false;
	...
	public var iconP2Alt:HealthIcon2;
	public var iconP3:HealthIcon3;
	...
	var tentlight1:FlxSprite;
	var tentlight2:FlxSprite;
	var tentlight3:FlxSprite;
	...
	override public function create()
	{
		...
		if (SONG.song.toLowerCase() == 'meltdown' && ClientPrefs.chromaticaberration)
			{
		...
		switch (curStage)
		{
			...
			case 'lot':
				skyBG = new FlxSprite(-600, -300).loadGraphic(Paths.image('samey/Clear_Sky'));
				if (SONG.song.toLowerCase() == 'meltdown')
				{
					skyBG = new FlxSprite(-600, -300).loadGraphic(Paths.image('samey/corruptedSky'));
				}
				skyBG.antialiasing = true;
				skyBG.scrollFactor.set(0.1, 0.1);
				add(skyBG);

				if (SONG.song.toLowerCase() == 'alerted')
				{
					error = new FlxSprite(-600, -300).loadGraphic(Paths.image('samey/black'));
					error.antialiasing = true;
					error.scrollFactor.set(0.1, 0.1);
					error.alpha = 0;
					add(error);
				}
				
				airship = new FlxSprite(1500, -200).loadGraphic(Paths.image('samey/AirShipMoving'));
				airship.antialiasing = true;
				airship.scrollFactor.set(0.3, 0.3);
				if (SONG.song.toLowerCase() == 'meltdown')
				{
				...
				clouds = new FlxSprite(-300, -200).loadGraphic(Paths.image('samey/Clouds'));
				...
				var stageFront:FlxSprite = new FlxSprite(-750, -300).loadGraphic(Paths.image('samey/Parking_Building'));
				if (SONG.song.toLowerCase() == 'meltdown')
				{
					stageFront = new FlxSprite(-750, -300).loadGraphic(Paths.image('samey/corruptedBG'));
				}
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
			...
			case 'demolished':
				var stars:FlxSprite = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/NightSky'));
				...
				var ground:FlxSprite = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Background_Night'));
				ground.antialiasing = true;
				ground.scrollFactor.set(0.75, 0.75);
				ground.screenCenter();
				ground.active = false;
				add(ground);

				var moon:FlxSprite = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Moon'));
				moon.antialiasing = true;
				moon.scrollFactor.set(0.79, 0.79);
				moon.screenCenter();
				moon.active = false;
				moon.x += 390;
				moon.y -= 520;
				...
				tentlight1 = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Background_Night_TentLight'));
				tentlight1.antialiasing = true;
				tentlight1.scrollFactor.set(0.745, 0.745);
				tentlight1.screenCenter();
				tentlight1.active = false;
				tentlight1.alpha = 0.6;
				tentlight1.angle += 46;
				tentlight1.x -= 480;
				tentlight1.y -=214;
				add(tentlight1);

				tentlight2 = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Background_Night_TentLight'));
				tentlight2.antialiasing = true;
				tentlight2.scrollFactor.set(0.745, 0.745);
				tentlight2.screenCenter();
				tentlight2.active = false;
				tentlight2.alpha = 0.6;
				tentlight2.angle += 30;
				tentlight2.x -= 587;
				tentlight2.y -=267;
				add(tentlight2);

				tentlight3 = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Background_Night_TentLight'));
				tentlight3.antialiasing = true;
				tentlight3.scrollFactor.set(0.745, 0.745);
				tentlight3.screenCenter();
				tentlight3.active = false;
				tentlight3.alpha = 0.6;
				tentlight3.angle -= 23;
				tentlight3.x -= 851;
				tentlight3.y -=282;
				add(tentlight3);

				var tent:FlxSprite = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Background_Night_Tent'));
				tent.antialiasing = true;
				tent.scrollFactor.set(0.75, 0.75);
				tent.screenCenter();
				tent.x -= 700;
				tent.y += 21;
				tent.active = false;
				...
				var front:FlxSprite = new FlxSprite().loadGraphic(Paths.image('demolishedNightAssets/Parking_Building_Night'));
...
/*		// "GLOBAL" SCRIPTS
    ...
    if(doPush)
      luaArray.push(new FunkinLua(Asset2File.getPath(luaFile)));
		...
		if(SONG.song.toLowerCase() == "fatigue") {
            remove(gfGroup);
		}
		...
		healthBarBG = new AttachedSprite('barred');
		...
		iconP3 = new HealthIcon3(dad.healthIcon, false);
		iconP3.y = healthBar.y - 110;
		iconP3.x = healthBar.x - 75;
		...
		iconP2Alt = new HealthIcon2(dad.healthIcon, false);
		iconP2Alt.y = healthBar.y - 75;
		iconP2Alt.visible = !ClientPrefs.hideHud;
		iconP2Alt.alpha = ClientPrefs.healthBarAlpha;
		if (SONG.song.toLowerCase() == 'meltdown')
		{
			remove(iconP2);
			add(iconP2Alt);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'alerted':
					startVideo('noyoudont');

				case 'meltdown': //just doing this for now so I dont want to worry abt playing them simultaneously, and yes I did try(Playstate:Line1530) 
					startVideo('okaypsych');
	...
	public function reloadHealthBarColors() {
		healthBar.createImageEmptyBar(Paths.image('barred'), FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]));
		healthBar.createImageFilledBar(Paths.image('bargreen'), FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]));
		healthBar.updateBar();
	}
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function balls(name:String):Void {
			#if VIDEOS_ALLOWED
			var foundFile:Bool = false;
			var fileName:String = #if MODS_ALLOWED Paths.modFolders('videos/' + name + '.' + Paths.VIDEO_EXT); #else ''; #end
			#if sys
	...
	function movelights() {
		FlxTween.tween(tentlight1,{angle: tentlight1.angle + 5},1.4);
		FlxTween.tween(tentlight2,{angle: tentlight2.angle - 10},1.4);
		FlxTween.tween(tentlight3,{angle: tentlight3.angle - 7},1.4,{ease: null, onComplete: function(flxTween:FlxTween) 
			{ 
				FlxTween.tween(tentlight1,{angle: tentlight1.angle - 5},1.4);
				FlxTween.tween(tentlight2,{angle: tentlight2.angle + 10},1.4);
				FlxTween.tween(tentlight3,{angle: tentlight3.angle + 7},1.4,{ease: null, onComplete: function(flxTween:FlxTween) 
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		black.screenCenter();
		add(black);
		FlxTween.angle(black, -360, 45, 1.5, {ease: FlxEase.expoInOut});
		FlxTween.tween(black,{"scale.x": 0.1,"scale.y": 0.1},1 ,{ease: FlxEase.expoInOut, onComplete: function(flxTween:FlxTween) 
		{ 
			FlxTween.tween(black,{x: black.x + 1380},1 ,{ease: FlxEase.expoInOut, onComplete: function(flxTween:FlxTween) 
			{ 
				if (dialogueBox != null)
				{
					add(dialogueBox);
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if(eyeTween != null) eyeTween.active = false;
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if(eyeTween != null) eyeTween.active = true;
	...
	override public function update(elapsed:Float)
	{
		...
		var mult:Float = FlxMath.lerp(1, iconP2Alt.scale.x, CoolUtil.boundTo(1 - (elapsed * 9), 0, 1));
		iconP2Alt.scale.set(mult, mult);
		...
		iconP2Alt.x = healthBar.x + (healthBar.width * (FlxMath.remapToRange(healthBar.percent, 0, 100, 100, 0) * 0.01)) - (150 * iconP2Alt.scale.x) / 2 - iconOffset * 2;
		...
		if (SONG.song.toLowerCase() == 'meltdown')
			{
				...
				camHUD.angle = FlxG.random.float(-3, 3);
				camGame.shake(0.02, 0.02);
				camHUD.shake(0.02, 0.02);
				yourmom = 0.015;
				FlxTween.tween(this, {yourmom: 0}, 0.05);
				if (healthBar.percent > 20 && !note.isSustainNote)
				{
					health -= 0.05;
				}
			}
			...
			if(note.hitCausesMiss) {
				...
				switch(note.noteType) {
					...
					case 'Hallucination Note':
						var eye:FlxSprite = new FlxSprite(playerStrums.members[note.noteData].x, playerStrums.members[note.noteData].y);
						eye.frames = Paths.getSparrowAtlas('mechanics/Meltdown_Gimmick_Eye', 'shared');
						eye.animation.addByPrefix('idle', 'Meltdown_Gimmick_Eye_1', 24, false);
						eye.animation.play('idle');
						eye.x += -(eye.width / 2) + 54;
						eye.y += -(eye.height / 2) + 64;
						eye.alpha = 0.8;
						add(eye);
						eye.cameras = [camHUD];
						if(!ClientPrefs.downScroll)
						{
							eyeTween = FlxTween.tween(eye, {y: eye.y + FlxG.height * 1.1}, 2.9, {ease: FlxEase.circIn, onComplete: function (twn:FlxTween) {
								remove(eye);
								eyeTween = null;
							}});
						} else
						...
						FlxG.sound.play(Paths.sound('Meltdown_EyeOfDysno_Sound', 'shared'));
				}
	...
	function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.noteSplashes && note != null) {
			...
			if(strum != null) {
				...
				{
					case 0: 
						spawnNoteSplash(strum.x + 55, strum.y + 63, note.noteData, note);
					case 1:
						spawnNoteSplash(strum.x + 60, strum.y + 60, note.noteData, note);
					case 2:
						spawnNoteSplash(strum.x + 48, strum.y + 57, note.noteData, note);
					case 3:
						spawnNoteSplash(strum.x + 64, strum.y + 53, note.noteData, note);
					default:
						spawnNoteSplash(strum.x + 60, strum.y + 55, note.noteData, note);
				}
			}
		}
	}
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		var skin:String = 'Immunus_Showdown_Note_Splash';
